...
public class IndividualRitualMenu extends ChestMenu {
    ...
    private void initMenu(ItemStack output) {
        ...
        addItem(10, required[5], (player, slot, item, cursor, action) -> false);
        for(int i = 0; i < 9; i++) {
            if(i==5) continue;
            int slot;
            if(i<4)
                ...
                slot=14+i;
            addItem(slot, required[i], (player, clickSlot, item, cursor, action) -> false);
        }
        addItem(16, output, (player, clickSlot, item, cursor, action) -> false);
    }
}
